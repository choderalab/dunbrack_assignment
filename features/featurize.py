...
def _parse_arguments(**kwargs):
    ...
    if not kwargs:
        ...
        parser.add_argument(
            ...
            nargs='?',
            type=str,
            help='A MDAnalysis.core.universe.Universe object of the input structure (a pdb file or a simulation trajectory).'
        )
...
def featurize(**kwargs):
    ...
    Returns
    -------
    key_res : dict of variou groups of features
    ...
    #from kinomodel.features import query_klifs (something like this)
    
    
    # JG (temperary)
    from features import query_klifs
    from features import identify_protein_feature_atoms
    from features import calc_protein_feature_values
    ...
    if args.feature == "conf":
        ...
        key_res = identify_protein_feature_atoms.key_klifs_residues(klifs.numbering)
        
        # for ddr1
        #key_res={'group2': [0, 0, 0, 52, 69], 'group3': [180, 181, 182, 73]} # hardcode for ddr1, zeros are placeholders for irrelevant residues
        
        # option 1: get the key atom indices for feature calculation
        #(dih, dis) = identify_protein_feature_atoms.identify_atom_indices(args.coord, key_res)
        # option 2: get the feature values
        dihedrals, distances = calc_protein_feature_values.compute_simple_protein_features(args.coord, key_res)
        #return key_res, dih, dis
        return dihedrals, distances

    elif args.feature == "interact": # TODO: need to update
        klifs = query_klifs.query_klifs_database(args.pdb, args.chain)
        mean_dist = inf.compute_simple_interaction_features(args.pdb, args.chain, args.coord, klifs.ligand, klifs.numbering)
        return mean_dist

    elif args.feature == "both": # TODO: need to update
        klifs = query_klifs.query_klifs_database(args.pdb, args.chain)
        key_res = identify_protein_feature_atoms.key_klifs_residues(klifs.numbering)
        (dihedrals, distances) = identify_protein_feature_atoms.compute_simple_protein_features(args.pdb, args.chain, args.coord, klifs.numbering)
