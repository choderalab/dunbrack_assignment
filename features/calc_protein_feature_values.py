...
def compute_simple_protein_features(traj, key_res):
    ...
    dih[0][0] = int(topology.select(f"chainid 0 and residue {key_res['group3'][0]-1} and name C")[0]) # xxDFG C
    dih[0][1] = int(topology.select(f"chainid 0 and residue {key_res['group3'][0]} and name N")[0]) # xDFG N
    dih[0][2] = int(topology.select(f"chainid 0 and residue {key_res['group3'][0]} and name CA")[0]) # xDFG CA
    dih[0][3] = int(topology.select(f"chainid 0 and residue {key_res['group3'][0]} and name C")[0]) # xDFG C
    dih[1][0] = dih[0][1] # xDFG N
    dih[1][1] = dih[0][2] # xDFG CA
    dih[1][2] = dih[0][3] # xDFG C
    dih[1][3] = int(topology.select(f"chainid 0 and residue {key_res['group3'][1]} and name N")[0]) # DFG-Asp N

    # dihedral 2 & 3: DFG-Asp Phi & Psi
    dih[2][0] = dih[0][3] # xDFG C
    dih[2][1] = dih[1][3] # DFG-Asp N
    dih[2][2] = int(topology.select(f"chainid 0 and residue {key_res['group3'][1]} and name CA")[0]) # DFG-Asp CA
    dih[2][3] = int(topology.select(f"chainid 0 and residue {key_res['group3'][1]} and name C")[0]) # DFG-Asp C
    dih[3][0] = dih[2][1] # DFG-Asp N
    dih[3][1] = dih[2][2] # DFG-Asp CA
    dih[3][2] = dih[2][3] # DFG-Asp C
    dih[3][3] = int(topology.select(f"chainid 0 and residue {key_res['group3'][2]} and name N")[0]) # DFG-Phe N

    # dihedral 4 & 5: DFG-Phe Phi & Psi
    dih[4][0] = dih[2][3] # DFG-Asp C
    dih[4][1] = dih[3][3] # DFG-Phe N
    dih[4][2] = int(topology.select(f"chainid 0 and residue {key_res['group3'][2]} and name CA")[0]) # DFG-Phe CA
    dih[4][3] = int(topology.select(f"chainid 0 and residue {key_res['group3'][2]} and name C")[0]) # DFG-Phe C
    dih[5][0] = dih[4][1] # DFG-Phe N
    dih[5][1] = dih[4][2] # DFG-Phe CA
    dih[5][2] = dih[4][3] # DFG-Phe C
    dih[5][3] = int(topology.select(f"chainid 0 and residue {key_res['group3'][2]+1} and name N")[0]) # DFG-Gly N

    # dihedral 6: DFG-Phe Chi1
    dih[6][0] = dih[3][3] #DFG-Phe N
    dih[6][1] = dih[4][2] #DFG-Phe CA
    dih[6][2] = int(topology.select(f"chainid 0 and residue {key_res['group3'][2]} and name CB")[0]) # DFG-Phe CB
    dih[6][3] = int(topology.select(f"chainid 0 and residue {key_res['group3'][2]} and name CG")[0]) # DFG-Phe CG

    ### distances
    ## Dunbrack distances D1, D2
    dis[0][0] = int(topology.select(f"chainid 0 and residue {key_res['group3'][3]} and name CA")[0]) # ExxxX CA
    dis[0][1] = int(topology.select(f"chainid 0 and residue {key_res['group3'][2]} and name CZ")[0]) # DFG-Phe CZ
    dis[1][0] = int(topology.select(f"chainid 0 and residue {key_res['group2'][3]} and name CA")[0]) # K in beta III CA 
    ...
    # calculate dihedrals and distances
